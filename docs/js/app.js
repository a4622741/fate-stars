...
function saveGame(){
  try{
    ...
    [...TIANGANG,...DISHAT].forEach(s=>{if(s.status==='recruited'&&s.id&&!['alfar','orange'].includes(s.id))starOv[s.type+'_'+s.num]={status:s.status,name:s.name,id:s.id};});
    const data={
...
function loadGame(){
  try{
    ...
    G.partyIds=data.partyIds||['alfar','orange'];
    ...
    if(data.starOv){
      Object.entries(data.starOv).forEach(([k,v])=>{
        ...
        if(s){s.status=v.status;s.name=v.name;s.id=v.id;}
      });
    }
...
// ═══ API ═══
const SYS=`你是西方奇幻版水滸傳文字RPG故事引擎。靈感來源：施耐庵《水滸傳》與Konami《幻想水滸傳》系列。只輸出純JSON，零Markdown，零額外文字。

═══ 世界觀 ═══
艾爾薩大陸，帝國崩裂後十二王國割據。腐敗蔓延，義人無處容身。108顆命運之星降世——天意如此，聚星者將終結輪迴。
天父星（先行者/晁蓋位）：最初感知星辰降世之人，已在途中倒下，留下未竟之志。
主角艾爾法😒（天魁星/宋江位）：前城衛，銀髮，面無表情，因堅持正義而被解僱——逼上梁山。不是英雄，是被命運選中的普通人。
橘子🐈😒（地魁星）：五枚銅幣買來的布偶貓，只會喵叫，知力99。命運之錨——她選擇了誰，108星便跟隨誰。

═══ 故事原則（水滸傳＋幻想水滸傳）═══
【逼上梁山】每位星辰之人都有「被體制逼迫」的過去。冤屈、背叛、迫害——他們不是自願成為反抗者，是別無選擇。描寫新角色時必須展現這層悲劇。
【義氣為核】義（righteousness）高於一切。角色間的信任不靠血緣，靠共患難。背叛是最嚴重的劇情事件，不可輕率使用但必須存在。
【招募即劇情】招募新星辰不是支線——每個人加入的故事就是主線。六種招募模式交替使用：
  ① 逼上梁山型：被冤枉/迫害，走投無路加入
  ② 義氣相投型：目睹玩家的正義行為，主動歸附
  ③ 計謀招攬型：需要設局/說服/滿足條件才能拉攏
  ④ 比武收服型：必須在戰鬥/比試中擊敗對方
  ⑤ 連環引薦型：招募A後，A引介B，B又引介C
  ⑥ 時機限定型：錯過特定窗口就永遠失去
【連環敘事】一個角色的故事自然引出2-3個新角色的線索。每次遇到重要NPC時，用info或sp暗示其他星辰的存在。
【天罡地煞分量】天罡36星是將領級——每人有完整的背景弧線、專屬劇情。地煞72星是專才——鐵匠、廚師、醫師、密探、商人、學者。不是每個星辰都會戰鬥，但每個都不可或缺。
...
B. 凡敘述涉及好感變動，fa必須填[{id,delta,reason}]。
C. 凡敘述涉及道具獲得/失去/購買，iv必須填。買道具→iv.add，買裝備→iv.purchase，消耗→iv.remove。範例：買了魚→"iv":{"add":[{"n":"鮮魚","t":"食材","q":"×1"}]}。
D. 凡敘述涉及HP變動，hp必須填。
E. 凡敘述涉及任務變動，qt必須填。
F. 凡敘述涉及時間推進，tm必須填。
...
async function sendChoice(txt){
  ...
  // 自動壓縮：超過60輪靜默壓縮
  if(G.history.length>120&&!G.autoCompressing){
...
function handleStarPresence(sp){
  ...
  if(isSpecial){
    ...
    // 自動加入情報板
    addIntel({id:'orange_star_'+sp.num,title:`第${sp.num}星・${sp.name||'?'}`,content:sp.hint||`橘子感知到此人身上有${starInfo}的氣息。`,src:'橘子感知',rel:4,cat:'人物',orange:true,related:`${sp.type}第${sp.num}星`});
  }
}
...
// ═══ PARTY MANAGEMENT ═══
function getCharData(id){return PARTY.find(c=>c.id===id)||G.extraParty.find(c=>c.id===id);}
function allParty(){return G.partyIds.map(id=>getCharData(id)).filter(Boolean);}
...
function leaveParty(id,ev){
  ...
  G.partyIds=G.partyIds.filter(x=>x!==id);
  renderBoth('party');renderBoth('stars');saveGame();
...
function renderResp(d){
  ...
  tickBondCooldowns();
  if(d.fa){(d.fa||[]).forEach(f=>{if(f.id&&f.delta){setFavor(f.id,f.delta);if(f.delta>0)showToast(`${getCharData(f.id)?.name||f.id} 好感 +${f.delta}`,'ok');else showToast(`${getCharData(f.id)?.name||f.id} 好感 ${f.delta}`,'inf');}});renderChanged('party');}
  scrollD();
...
function renderAll(){
  ...
  renderBoth(vis);
  if(G.currentChoices?.length)renderChoices(G.currentChoices,false);
  // 用 RAF 批次渲染其餘分頁（不阻塞當前畫面）
  requestAnimationFrame(()=>{
    ['party','stars','inv','quest','intel','log'].forEach(t=>{
      if(t!==vis){markDirty(t);renderBoth(t);}
    });
  });
}
...
function restoreAllHP(){
  allParty().forEach(c=>{
    const hp=getHP(c.id);
    if(hp.cur<hp.max) applyHPChange([{id:c.id,delta:hp.max-hp.cur,reason:'完全休息'}]);
  });
}

function restAllHP(){
  const injured=allParty().filter(c=>{const hp=getHP(c.id);return hp.cur<hp.max;});
  if(!injured.length){showToast('所有人HP已滿','inf');return;}
  injured.forEach(c=>{
    const hp=getHP(c.id);
    applyHPChange([{id:c.id,delta:Math.floor(hp.max*0.4),reason:'紮營休息'}]);
  });
...
async function testAPI(){
  ...
  }catch(e){if(w){w.textContent='✗ 網路錯誤';w.style.color='var(--red)';}}
}
...
function renderBoth(tab){
  let h;
  const cacheable=['stars']; // log moved out - changes every response; party never cached
...
function switchTab(el,pf,name){
  ...
  G.activeTab=name;
  // 切換時才渲染（lazy render）
  markDirty(name);renderBoth(name);
}
...
function openStar(type,num){
  ...
  document.getElementById('modal-inner').innerHTML=`<div class="mtop"><div class="mscol"><div class="mtp">${type}</div><div class="mnm">第${num}星</div><div class="mst">${s.star}</div></div><div class="micol"><div class="mname">${iU?'？？':s.name}</div><div class="msub2">${type}・第${num}星・${s.star}</div><div class="mstat ${sc}">${st}</div></div></div><div class="mbody">${body}</div>`;
  document.getElementById('detail-modal').classList.add('open');
}
...
async function autoCompressHistory(){
  ...
  try{
    ...
    saveGame();
  }catch(e){console.warn('autoCompress failed:',e);}
}
...
function importSave(){
  ...
  try{
    ...
    if(loadGame()){
      ...
      renderStoryFromData();
      renderBoth('party');renderBoth('stars');renderBoth('log');
      showToast('✦ 存檔匯入成功','ok');
    }
  }catch(e){
...
async function testConn(){
  ...
  try{
    ...
    if(r.ok){res.textContent='✓ 連接成功';res.style.color='var(--grn)';}
...
applyResp();
renderBoth('party');renderBoth('stars');renderBoth('quest');renderBoth('intel');
